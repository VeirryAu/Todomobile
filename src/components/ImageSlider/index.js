...
export default class Slideshow extends Component {
  ...
  _next () {
    ...
    const pos = position === this.props.dataSource.length - 1 ? 0 : position + 1
    this._move(pos)
  ...
  _prev () {
    const { position } = this.state
    const pos = position === 0 ? this.props.dataSource.length - 1 : position - 1
    this._move(pos)
    this.setState({ position: pos })
  }
  ...
  handleScroll (event, width) {
    this.setState({
      position: event.nativeEvent.contentOffset.x / width
    })
  }
      ...
      >
        ...
        >
          {this.props.dataSource.map((image, index) => {
            const imageObject = typeof image.url === 'string' ? { uri: image.url } : { uri: image.url }
            const textComponent = (
